...
let rec string_of_exval = function
  ...
  | DProcV _ -> "<dfun>"
  | TupleV (left, right) ->
      "(" ^ string_of_exval left ^ ", " ^ string_of_exval right ^ ")"
...
let rec apply_prim op arg1 arg2 =
  ...
  | Cons, _, ListV ((DProcV _) :: _) ->
      err "Element must be dynamic function: ::"
  | Cons, (ProcV _ as p), ListV ((ProcV _) :: _ as xs) -> ListV (p :: xs)
  | Cons, _, ListV ((ProcV _) :: _) ->
      err "Element must be non-dynamic function: ::"
...
and eval_exp env = function
  | Var x -> (
    try Environment.lookup x !env with Environment.Not_bound ->
  ...
  | LazyBinOp (op, exp1, exp2) -> apply_prim_lazy op env exp1 exp2
  ...
  | MatchExp (target, head, tail, exp1, exp2) -> (
      if head = tail then
        ...
        let target_value = eval_exp env target in
        match target_value with
        | ListV [] -> eval_exp env exp1
        | ListV (x :: xs) ->
            let newenv =
              ref
                (Environment.extend head x
                   (Environment.extend tail (ListV xs) !env))
            in
            eval_exp newenv exp2
        | _ -> err "Pattern match target must be list" )
  | LoopExp (id, e1, e2) ->
      let loop_letrec =
        LetRecExp ("__loop__", id, e2, AppExp (Var "__loop__", e1))
      in
      eval_exp env loop_letrec
  | RecurExp e ->
      let loop_recur = AppExp (Var "__loop__", e) in
      eval_exp env loop_recur
...
let eval_decl env = function
  ...
  | RecDecl (id, _, e) ->
      recur_check e ;
      let dummyenv = ref Environment.empty in
      let newenv = Environment.extend id (eval_exp dummyenv e) env in
      dummyenv := newenv ;
      let v = eval_exp dummyenv e in
      (id, Environment.extend id v env, v)
